...
namespace Catfood.Shapefile
{
    ...
    public class Shapefile : IDisposable, IEnumerator<Shape>, IEnumerable<Shape>
    {
        ...
        /// to any one of these three files to open the Shapefile.
        /// </summary>
        /// <param name="dbfStream">Stream to the .shp, .shx or .dbf file for this Shapefile</param>
        /// <param name="mainStream">Stream to the .shp, .shx or .dbf file for this Shapefile</param>
        /// <param name="indexStream">Stream to the .shp, .shx or .dbf file for this Shapefile</param>
        /// <exception cref="ObjectDisposedException">Thrown if the Shapefile has been disposed</exception>
        /// <exception cref="ArgumentException">Thrown if the path parameter is empty</exception>
        /// <exception cref="FileNotFoundException">Thrown if one of the three required files is not found</exception>
        public Shapefile(Stream mainStream, Stream indexStream, Stream dbfStream) 
            : this(mainStream, indexStream, dbfStream, ConnectionStringTemplateJet) {}
        ...
        /// <exception cref="ArgumentException">Thrown if the path parameter is empty</exception>
        /// <exception cref="FileNotFoundException">Thrown if one of the three required files is not found</exception>
        public Shapefile(Stream mainStream, Stream indexStream, Stream dbfStream, string connectionStringTemplate)
        {
            if (connectionStringTemplate == null)
            {
                throw new ArgumentNullException("connectionStringTemplate");
        ...
        private void OpenDb()
        {
            ...
            if (Path.GetFileNameWithoutExtension(safeDbasePath).Length > 8)
            {
                ...
                try
                {
                    System.IO.File.Delete(initialTempFile);
                }
                catch { }
                ...
                // copy over the DB
                System.IO.File.Copy(_shapefileDbasePath, _shapefileTempDbasePath, true);
        ...
        /// <param name="directory">relative path of .dbf for this Shapefile</param>
        public void OpenDb(string directory)
        {
            // The drivers for DBF files throw an exception if the filename 
            // is longer than 8 characters - in this case create a temp file
            // for the DB
            string safeDbasePath = directory;
            if (Path.GetFileNameWithoutExtension(safeDbasePath).Length > 8)
            {
                // create/delete temp file (we just want a safe path)
                string initialTempFile = Path.GetTempFileName();
                ...
                // copy over the DB
                System.IO.File.Copy(directory, _shapefileTempDbasePath, true);
                safeDbasePath = _shapefileTempDbasePath;
            }
        ...
        private void CloseDb()
        {
            ...
            {
                if (System.IO.File.Exists(_shapefileTempDbasePath))
                {
                    try
                    {
                        System.IO.File.Delete(_shapefileTempDbasePath);
                    }
                    catch { }
                }
        ...
        public Shape Current
        {
            get 
            {
                ...
                if (!RawMetadataOnly)
                {
                    ...
                    //{
                    //    //Check it the value for the field is numeric
                    //    decimal number;
                    //    if (Decimal.TryParse(_dbReader.GetValue(i).ToString(), out number))
                    //    {
